...
def readNetlist(file, netporting):
    ...
    for line in file.readlines():
        ...
        gate = bits.pop(0)
        ports = map(int, bits)
        ports = [p + netporting for p in ports]
...
# read netlists
nets1, inputs1, outputs1, mapping1, gates1 = readNetlist(open("xor2.net", "r"), 2)
nets2, inputs2, outputs2, mapping2, gates2 = readNetlist(open("xor2_nand.net", "r"), 2 + nets1)


class Caching(object):
    """ If ITE(f,g,h) has been calculated before, immediately return the cached result. """

    def __init__(self, _fn):
        self._fn = _fn
        self.memo = {}

    def __call__(self, *var, **vars):
        import cPickle
        str_hash = cPickle.dumps(var, 1) + cPickle.dumps(vars, 1)
        # Search for calculated functions.
        if not self.memo.has_key(str_hash):
            self.memo[str_hash] = self._fn(*var, **vars)
...
def ITE(f, g, h):
    ...
    x = max(topVar(f), topVar(g), topVar(h))
...
def Cofactors(f, x):
    ...
    if x > topVar(f): return f, f

    if x == topVar(f): return f[1], f[2]  # A,B
